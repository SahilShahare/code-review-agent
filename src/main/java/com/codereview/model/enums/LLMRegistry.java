...
public enum LLMRegistry {
    ...
    private static final Map<String, LLMRegistry> LOOKUP = Arrays.stream(values())
            .flatMap(llm ->
                    Arrays.stream(llm.modelClass.getEnumConstants())
                            .map(e -> Map.entry(e.id(), llm)))
            .collect(Collectors.toUnmodifiableMap( Map.Entry::getKey, Map.Entry::getValue));

    private final Class<? extends LLMModel> modelClass;
    private final Class<? extends LLMClient> clientClass;

    <E extends Enum<E> & LLMModel>
    LLMRegistry(Class<E> modelClass, Class<? extends LLMClient> clientClass) {
