...
public class ReviewContextBuilder {
    ...
    private static final Set<String> supportedExtensions = Language.getAllExtensions();
    private final static String SOURCE_UNAVAILABLE = "(source unavailable -- declared outside the parsed source tree)";
    private final static String PROMPT_DIRECTORY = "prompts/";
    private final static String COMBINED_SECTION_TEMPELATE ="""
                --- CHANGED FILES (DIFFS) ---
                %s
                --- CALL GRAPH (changed methods, their calless, their callers) ---
                %s""";
    ...
    public String buildPrompt(Map<String, String> diffsByFile) throws IOException {
        ...
        Set<MethodSignature> changedMethods = callGraph.allMethods().stream()
        ...
        String combinedSection = COMBINED_SECTION_TEMPELATE.formatted(diffSection, graphSection);

        StringBuilder sourceAppendix = new StringBuilder();
        sourceByCanonicalId.forEach((id, source) ->
    ...
    private void registerSource(MethodSignature sig, Map<String, String> sourceByCanoncialId) {
        sourceByCanoncialId.computeIfAbsent(sig.canonicalId(), id -> {
