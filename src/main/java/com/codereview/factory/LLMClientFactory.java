...
public class LLMClientFactory {
    ...
    public static LLMClient build(String modelId) {
        try {
            LLMRegistry registry = LLMRegistry.getLLMRegistryFromModelId(modelId)
                    .orElseThrow(() ->
                            new IllegalArgumentException( "Invalid model id '" + modelId +
                                    "'. Supported models: " + LLMRegistry.allModels()));
            return switch (registry) {
                case GEMINI ->
                        createGeminiClient(modelId);
            };
        } catch (Exception e) {
    ...
    private static LLMClient createGeminiClient(String modelId) throws ReflectiveOperationException {
        ...
        if (apiKey == null || apiKey.isBlank()) {
            throw new IllegalStateException(
                    "Environment variable GEMINI_API_KEY is not set or is blank");
        }
        GeminiModel model = (GeminiModel) GEMINI.getModel(modelId)
                .orElseThrow(() -> new IllegalArgumentException(
                        "Unknown Gemini model: " + modelId));
        return GEMINI.getClientClass()
